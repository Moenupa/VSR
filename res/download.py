...
if __name__ == '__main__':
    ...
    files = [
        f for f in download_plan['files'].keys()
        if int(hashlib.md5(f.encode('utf-8')).hexdigest(), 16) %
    ...
    for f in files:
        fname, ext = f.split('.')
        out_f = '%s%04i.%s' % (str(fname[:-2]), file_index[str(
            fname[-2:])], ext)

        if os.path.exists(out_f) and md5sum(
                out_f) == download_plan['files'][f]:
